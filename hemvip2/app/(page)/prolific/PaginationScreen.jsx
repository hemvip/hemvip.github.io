	...
	return (
		...
		<div className="flex flex-col w-screen h-screen overflow-hidden">
			<div className="h-full flex-grow p-4">
				<div className="relative h-full w-full p-9 bg-blue-500 flex justify-center">
					<AnimatePresence initial={false} custom={currentPage}>
						...
						>
							<div className="bg-white rounded-lg h-full w-full">{pages.find((page) => page.id === currentPage)?.content}</div>
						</motion.div>
					</AnimatePresence>
			...
			<div className="flex justify-center p-4 z-10 flex-none">
				<button disabled={currentPage === 1} onClick={prevPage} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md mr-2 disabled:opacity-50">
					Previous
				</button>
				<button disabled={currentPage === pages.length} onClick={nextPage} className="px-4 py-2 bg-blue-500 text-white rounded-md disabled:opacity-50">
					Next
				</button>
			</div>
		</div>
