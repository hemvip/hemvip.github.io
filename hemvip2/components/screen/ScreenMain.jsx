...
export function ScreenMain({ currentPage, setNext }) {
	...
	const video1URL = page.type === "video" || page.type === "check" ? page.videos[0].url : "/gesture_video.mp4"
	const video2URL = page.type === "video" || page.type === "check" ? page.videos[1].url : "/gesture_video.mp4"
				...
				<div className="w-full h-full flex justify-center align-middle gap-2 ">
					<div className="flex-1 h-full">
						<div className="h-full w-full relative flex items-center justify-center">
							<button onClick={handleSkipLeft} className="z-50 flex flex-row justify-center items-center gap-1 right-4 top-4 absolute button rounded-md bg-white px-2 py-1" title="Skip Video">
								<span className="w-5 h-5">
									<SkipIcon className="bg-red-500"></SkipIcon>
								</span>
								<span className="font-semibold flex-grow text-sm">Skip</span>
							</button>
							<video
								muted={false}
								autoPlay={false}
					...
					<div className="flex-1 h-full">
						<div className="h-full w-full relative flex items-center justify-center">
							<button onClick={handleSkipRight} className="z-50 flex flex-row justify-center items-center gap-1 right-4 top-4 absolute button rounded-md bg-white px-2 py-1" title="Skip Video">
								<span className="w-5 h-5">
									<SkipIcon className="bg-red-500"></SkipIcon>
								</span>
								<span className="font-semibold flex-grow text-sm">Skip</span>
